...
def structured_diagnosis_sections(session: Any | None) -> dict[str, str]:
    ...
    if control_report is not None and getattr(control_report, "enabled", False):
        control_text = str(getattr(control_report, "summary", "") or "已计算")
    ...
    return {
        ...
        "AI结论": str(getattr(diagnosis, "practical_conclusion", "") or "暂无"),
        "控制校正": control_text,
        "测量补扫": "；".join(getattr(diagnosis, "measurement_suggestions", []) or ["暂无"]),
        "故障证据": evidence,
...
def diagnosis_clipboard_text(session: Any | None) -> str:
    ...
    control_report = getattr(session, "control_compensation_report", None) if session is not None else None
    if control_report is None and diagnosis is not None:
        control_report = getattr(diagnosis, "control_compensation_report", None)
    if control_report is not None and getattr(control_report, "enabled", False):
        lines.append(f"控制校正: {getattr(control_report, 'summary', '')}")
