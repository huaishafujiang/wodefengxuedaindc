...
from system_analysis.analysis.control_compensation import (
    ControlCompensationReport,
    build_control_compensation_report,
    format_control_compensation_report_lines,
)
from system_analysis.io.serial_protocol import build_sweep_command
...
DEFAULT_KB_PATH = Path(__file__).resolve().parents[1] / "data" / "diagnosis_knowledge_base.json"
NOISE_FLOOR_RMS_V = 0.004
LOW_INPUT_RMS_V = 0.03
ADC_BIAS_CENTER_V = 1.65
MAX_STANDARD_SIMPLE_ORDER = 3
SMART_TARGET_CONFIDENCE = 0.90
SMART_MIN_SWEEP_HZ = 5.0
SMART_MIN_KEY_REGION_POINTS = 10
...
def _estimate_key_frequencies(
    ...
    if analysis_result is not None:
        ...
        if circuit_type in ("bandpass", "bandstop") and (
            center_hz is not None or cutoff_hz is not None or right_hz is not None
        ):
            if bandwidth_hz is None and cutoff_hz is not None and right_hz is not None:
                bandwidth_hz = max(right_hz - cutoff_hz, 0.0)
...
def _build_active_sweep_steps(
    ...
    def add_step(command: str, reason: str, suggestion: str | None = None) -> None:
        cmd = str(command or "").strip()
        if not cmd or cmd in seen_commands:
            return
        seen_commands.add(cmd)
        steps.append(ActiveSweepStep(command=cmd, reason=reason))
        suggestions.append(suggestion or f"{reason}，建议命令 {cmd}。")

    def inside_measured(frequency_hz: float | None) -> bool:
        return _frequency_inside_measured_range(features, frequency_hz)

    def point_count(start_hz: float, stop_hz: float) -> int:
        if omega is None:
            return 0
        return _count_points_in_hz_window(omega, start_hz, stop_hz)

    def boundary_needs_resweep(boundary: float | None) -> bool:
        if boundary is None or not np.isfinite(boundary) or boundary <= 0.0:
            return False
        if not inside_measured(boundary):
            return True
        return point_count(boundary * 0.70, boundary * 1.30) < 8

    def low_side_needs_extension() -> bool:
        if center_hz is None or not np.isfinite(center_hz) or center_hz <= 0.0:
            return True
        if inside_measured(left_hz):
            return False
        if features.freq_min_hz <= max(20.0, center_hz * 0.02):
            return False
        return point_count(features.freq_min_hz, min(features.freq_max_hz, center_hz * 0.70)) < 24

    def high_side_needs_extension() -> bool:
        if center_hz is None or not np.isfinite(center_hz) or center_hz <= 0.0:
            return True
        if inside_measured(right_hz):
            return False
        if features.freq_max_hz >= center_hz * 5.0:
            return False
        return point_count(max(features.freq_min_hz, center_hz * 1.30), features.freq_max_hz) < 24

    if circuit_type in ("lowpass", "highpass") and cutoff_hz is not None:
        if inside_measured(cutoff_hz) and point_count(cutoff_hz * 0.60, cutoff_hz * 2.50) >= 8:
            return suggestions, steps
        if inside_measured(cutoff_hz):
            start = max(features.freq_min_hz * 0.8, cutoff_hz * 0.60, 1.0)
            stop = min(max(features.freq_max_hz * 1.15, cutoff_hz * 2.0), cutoff_hz * 2.5)
        elif cutoff_hz < features.freq_min_hz:
            start = max(1.0, min(cutoff_hz * 0.50, features.freq_min_hz * 0.20))
            stop = max(features.freq_min_hz * 1.40, cutoff_hz * 2.50)
        else:
            start = max(1.0, min(features.freq_max_hz * 0.70, cutoff_hz * 0.60))
            stop = max(features.freq_max_hz * 1.60, cutoff_hz * 1.30)
        ...
        add_step(command, reason)
    elif circuit_type in ("bandpass", "bandstop"):
        for label, boundary in (("左侧 -3 dB 边界", left_hz), ("右侧 -3 dB 边界", right_hz)):
            if boundary is None or not np.isfinite(boundary) or boundary <= 0.0:
                continue
            if not boundary_needs_resweep(boundary):
                continue
            if not (features.freq_min_hz * 0.50 <= boundary <= features.freq_max_hz * 1.50):
                continue
            start = max(boundary * 0.70, 1.0)
            stop = max(boundary * 1.30, start + 1.0)
            step = _nice_step_hz((stop - start) / 32.0)
            command = _make_sweep_command(start, stop, step)
            reason = f"围绕{label} {boundary:.0f} Hz 加密补扫"
            add_step(command, reason, f"{reason}，确认带宽和边界斜率。")

        if center_hz is not None and np.isfinite(center_hz) and center_hz > 0.0:
            if low_side_needs_extension():
                start = max(20.0, min(features.freq_min_hz * 0.20, center_hz * 0.20))
                stop = max(features.freq_min_hz * 1.50, center_hz * 1.40)
                step = _nice_step_hz((stop - start) / 80.0)
                command = _make_sweep_command(start, stop, step)
                reason = f"低频侧边界未稳定，扩展到中心频率 {center_hz:.0f} Hz 左侧补扫"
                add_step(command, reason)
            if high_side_needs_extension():
                start = max(1.0, min(features.freq_max_hz * 0.75, center_hz * 0.70))
                stop = max(features.freq_max_hz * 1.60, center_hz * 5.0)
                step = _nice_step_hz((stop - start) / 120.0)
                command = _make_sweep_command(start, stop, step)
                reason = f"高频侧边界未稳定，扩展到中心频率 {center_hz:.0f} Hz 右侧补扫"
                add_step(command, reason)
...
def _window_step(start_hz: float, stop_hz: float, target_points: int) -> float:
    return _nice_step_hz(max((float(stop_hz) - float(start_hz)) / max(float(target_points), 1.0), 0.1))


def _measured_edge_is_hardware_floor(features: DiagnosisFeatures) -> bool:
    return features.freq_min_hz <= SMART_MIN_SWEEP_HZ * 1.15


def _build_goal_driven_sweep_steps(
    omega: Any,
    magnitude: Any,
    features: DiagnosisFeatures,
    circuit_type: str,
    ...
    left_hz: float | None,
    right_hz: float | None,
    existing_steps: list[ActiveSweepStep] | None = None,
) -> tuple[list[str], list[ActiveSweepStep]]:
    suggestions: list[str] = []
    steps: list[ActiveSweepStep] = []
    seen_commands: set[str] = set()
    for step in existing_steps or []:
        cmd = str(step.command or "").strip()
    ...
    def add_window(
        ...
        *,
        ...
        start = max(SMART_MIN_SWEEP_HZ, float(start_hz))
        stop = max(start + 1.0, float(stop_hz))
        if min_points and _count_points_in_hz_window(omega, start, stop) >= min_points:
            return
        command = _make_sweep_command(start, stop, _window_step(start, stop, target_points))
        _add_unique_sweep_step(steps, suggestions, seen_commands, command, reason)

    if circuit_type in ("lowpass", "highpass") and cutoff_hz is not None and np.isfinite(cutoff_hz) and cutoff_hz > 0.0:
        add_window(
            cutoff_hz * 0.45,
            cutoff_hz * 2.80,
            90,
            f"目标驱动补扫：围绕截止频率 {cutoff_hz:.0f} Hz 加密到置信度目标",
            min_points=SMART_MIN_KEY_REGION_POINTS,
        )
        if cutoff_hz < features.freq_min_hz and not _measured_edge_is_hardware_floor(features):
            add_window(
                min(cutoff_hz * 0.35, features.freq_min_hz * 0.25),
                max(cutoff_hz * 2.50, features.freq_min_hz * 1.30),
                80,
                f"目标驱动补扫：截止频率低于当前起点，向低频扩展确认 {cutoff_hz:.0f} Hz",
            )
        elif cutoff_hz > features.freq_max_hz:
            add_window(
                max(features.freq_max_hz * 0.70, cutoff_hz * 0.45),
                max(features.freq_max_hz * 2.20, cutoff_hz * 1.60),
                100,
                f"目标驱动补扫：截止频率高于当前终点，向高频扩展确认 {cutoff_hz:.0f} Hz",
            )

    if circuit_type in ("bandpass", "bandstop"):
        if left_hz is not None and np.isfinite(left_hz) and left_hz > 0.0:
            if left_hz < features.freq_min_hz and not _measured_edge_is_hardware_floor(features):
                add_window(
                    min(left_hz * 0.45, features.freq_min_hz * 0.30),
                    max(left_hz * 1.80, features.freq_min_hz * 1.25),
                    90,
                    f"目标驱动补扫：左侧 -3 dB 边界 {left_hz:.0f} Hz 低于当前起点，向低频扩展",
                )
            else:
                add_window(
                    left_hz * 0.55,
                    left_hz * 1.80,
                    70,
                    f"目标驱动补扫：加密左侧 -3 dB 边界 {left_hz:.0f} Hz",
                    min_points=SMART_MIN_KEY_REGION_POINTS,
                )
        elif center_hz is not None and np.isfinite(center_hz) and center_hz > 0.0 and not _measured_edge_is_hardware_floor(features):
            add_window(
                min(features.freq_min_hz * 0.30, center_hz * 0.05),
                max(features.freq_min_hz * 1.40, center_hz * 0.80),
                100,
                f"目标驱动补扫：左侧边界未找到，向中心频率 {center_hz:.0f} Hz 左侧扩展",
            )

        if right_hz is not None and np.isfinite(right_hz) and right_hz > 0.0:
            if right_hz > features.freq_max_hz:
                add_window(
                    max(features.freq_max_hz * 0.75, right_hz * 0.55),
                    max(features.freq_max_hz * 1.80, right_hz * 1.45),
                    110,
                    f"目标驱动补扫：右侧 -3 dB 边界 {right_hz:.0f} Hz 高于当前终点，向高频扩展",
                )
            else:
                add_window(
                    right_hz * 0.55,
                    right_hz * 1.80,
                    80,
                    f"目标驱动补扫：加密右侧 -3 dB 边界 {right_hz:.0f} Hz",
                    min_points=SMART_MIN_KEY_REGION_POINTS,
                )
        elif center_hz is not None and np.isfinite(center_hz) and center_hz > 0.0:
            add_window(
                min(features.freq_max_hz * 0.75, center_hz * 1.20),
                max(features.freq_max_hz * 1.60, center_hz * 5.0),
                120,
                f"目标驱动补扫：右侧边界未找到，向中心频率 {center_hz:.0f} Hz 右侧扩展",
            )
        ...
        if center_hz is not None and np.isfinite(center_hz) and center_hz > 0.0:
            add_window(
                ...
                center_hz * 1.80,
                80,
                f"目标驱动补扫：加密中心/峰值频段 {center_hz:.0f} Hz",
                min_points=14,
            )

    if confidence < SMART_TARGET_CONFIDENCE and features.peak_freq_hz > 0.0:
        add_window(
            features.peak_freq_hz * 0.55,
            features.peak_freq_hz * 1.90,
            80,
            f"目标驱动补扫：当前置信度 {confidence:.2f} 未达 {SMART_TARGET_CONFIDENCE:.2f}，复核主峰 {features.peak_freq_hz:.0f} Hz",
            min_points=16,
        )

    if best_fit is not None and best_fit.max_residual_db > 6.0:
        key_hz = best_fit.parameters.get("fc_hz") or best_fit.parameters.get("f0_hz") or center_hz or cutoff_hz
        if key_hz is not None and np.isfinite(key_hz) and key_hz > 0.0:
            add_window(
                key_hz * 0.50,
                key_hz * 2.00,
                90,
                f"目标驱动补扫：模型最大残差 {best_fit.max_residual_db:.1f} dB，复核拟合关键频段 {key_hz:.0f} Hz",
                min_points=18,
            )

    return _dedupe_texts(suggestions), steps[:4]
...
def _resweep_decision(
    ...
    if confidence < SMART_TARGET_CONFIDENCE:
    ...
    if key_inside_range and key_count is not None and key_count < 8:
        ...
        if circuit_type in ("lowpass", "highpass") and cutoff_hz is not None:
            reasons.append(
                ...
                f"{features.freq_min_hz:.0f}-{features.freq_max_hz:.0f} Hz，将自动生成扩展范围补扫命令"
            )
        else:
            reasons.append("估计关键边界超出当前扫频范围，将自动生成扩展范围补扫命令")

    blocking_faults = [item for item in fault_findings if item.rule_id == "pa1_bias_rail"]
    if blocking_faults:
        return (
            False,
    ...
    if reasons:
        return True, _dedupe_texts(reasons), True
    return (
        False,
...
def _strip_auto_resweep_claims(items: list[str]) -> list[str]:
    stripped: list[str] = []
    for item in items:
        text = str(item or "").strip()
        if not text:
            continue
        text = text.replace("，将自动生成扩展范围补扫命令", "")
        text = text.replace("；将自动生成扩展范围补扫命令", "")
        text = text.replace("将自动生成扩展范围补扫命令", "当前没有新的自动补扫命令")
        stripped.append(text)
    return _dedupe_texts(stripped)


def _no_auto_resweep_notes(
    features: DiagnosisFeatures,
    circuit_type: str,
    cutoff_hz: float | None,
    center_hz: float | None,
    ...
    if circuit_type in ("lowpass", "highpass") and cutoff_hz is not None and np.isfinite(cutoff_hz):
        if cutoff_hz < features.freq_min_hz and features.freq_min_hz <= lower_limit_hz * 1.05:
            notes.append(
                f"估计截止频率低于当前最低频点 {features.freq_min_hz:.0f} Hz；"
                "自动补扫已到 20 Hz 下限，暂时没有新的自动 SWEEP 命令。"
            )
        elif cutoff_hz > features.freq_max_hz:
            notes.append(
                f"估计截止频率高于当前最高频点 {features.freq_max_hz:.0f} Hz；"
                "自动补扫没有找到未覆盖的新频段，建议手动扩大终止频率或复测确认。"
            )

    if circuit_type in ("bandpass", "bandstop"):
        if left_hz is None or not np.isfinite(left_hz) or left_hz < features.freq_min_hz:
            if features.freq_min_hz <= lower_limit_hz * 1.05:
                notes.append(
                    f"左侧 -3 dB 边界低于当前最低频点 {features.freq_min_hz:.0f} Hz；"
                    "自动补扫已到 20 Hz 下限，暂时没有新的自动 SWEEP 命令。"
                )
            else:
                notes.append("左侧 -3 dB 边界仍未稳定覆盖，但自动补扫没有找到新的有效低频段。")
        if right_hz is None or not np.isfinite(right_hz) or right_hz > features.freq_max_hz:
            limit_note = ""
            if center_hz is not None and np.isfinite(center_hz) and center_hz > 0.0 and features.freq_max_hz >= center_hz * 5.0:
                limit_note = "当前最高频率已超过中心频率约 5 倍，"
            notes.append(
                f"右侧 -3 dB 边界高于当前最高频点 {features.freq_max_hz:.0f} Hz；"
                f"{limit_note}自动补扫暂时没有新的可执行高频命令。"
            )

    if not notes:
        notes.append(
            "当前诊断仍建议人工确认，但自动补扫器没有发现未覆盖的新频段；"
            "建议手动复测同一范围、检查幅值/接线，或在报告中标注边界超出当前量程。"
        )
...
def _practical_conclusion_and_experiments(
    ...
    priority_faults = [item for item in fault_findings if item.rule_id == "pa1_bias_rail"]
    if priority_faults:
        first = priority_faults[0]
        conclusion = (
        ...
        if circuit_type in ("bandpass", "bandstop"):
            conclusion = (
                ...
                f"范围内呈现{CIRCUIT_TYPE_LABELS.get(circuit_type, '')}趋势；中心频率约 {_format_hz(key_hz)}，"
                "但左右 -3 dB 边界没有被稳定覆盖，不能把等效传函当最终模型。"
            )
        else:
            conclusion = (
                f"当前数据只能证明被测对象在 {features.freq_min_hz:.0f}-{features.freq_max_hz:.0f} Hz "
...
def run_intelligent_diagnosis(
    ...
    if resweep_needed and actionable_resweep:
        ...
        goal_suggestions, goal_steps = _build_goal_driven_sweep_steps(
            omega,
            magnitude,
            features,
            circuit_type,
        ...
        sweep_steps = goal_steps
        measurement_suggestions.extend(goal_suggestions)
        if not sweep_steps:
            measurement_suggestions = _strip_auto_resweep_claims(measurement_suggestions)
            measurement_suggestions.extend(
                _no_auto_resweep_notes(
                    features,
                    circuit_type,
                    cutoff_hz,
                    center_hz,
    ...
    measurement_suggestions = _dedupe_texts(measurement_suggestions)
    if features.point_count < 12:
