...
from system_analysis.reporting.presentation import presentation_bode_series
...
def _nyquist_for_plot(result: Any, phase_display: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    mag = _series_or_empty(getattr(result, "mag_smooth", getattr(result, "mag_raw", [])))
    phase_display = _series_or_empty(phase_display)
    n = min(len(mag), len(phase_display))
    if n == 0:
        return np.asarray([], dtype=float), np.asarray([], dtype=float)
    h_jw = mag[:n] * np.exp(1j * np.radians(phase_display[:n]))
    return np.real(h_jw), np.imag(h_jw)
...
def _plot_contiguous_mask(ax, x, y, mask, *, color, lw, alpha, label=None, semilogx=False):
    x = np.asarray(x, dtype=float)
    y = np.asarray(y, dtype=float)
    ...
    x = x[:n]
    y = y[:n]
    mask = mask[:n] & np.isfinite(x) & np.isfinite(y)
    if not np.any(mask):
        return
    ...
    for chunk in np.split(indices, split_at):
        if len(chunk) == 0:
            continue
        segment_label = label if not label_used else None
        label_used = True
        if semilogx:
            ax.semilogx(x[chunk], y[chunk], color=color, lw=lw, alpha=alpha, label=segment_label)
        else:
            ax.plot(x[chunk], y[chunk], color=color, lw=lw, alpha=alpha, label=segment_label)


def _append_finite(target: list[np.ndarray], value: Any) -> None:
    array = _series_or_empty(value)
    if len(array):
        finite = array[np.isfinite(array)]
        if len(finite):
            target.append(finite)


def _set_y_limits_from_measured(ax, arrays: list[np.ndarray], *, min_span: float, margin_fraction: float) -> None:
    if not arrays:
        return
    ...
    if span < min_span:
        ...
        span = min_span
    margin = max(span * margin_fraction, min_span * 0.08)
...
def plot_sessions(fig, axes, sessions: list[Any], primary: Any | None = None) -> None:
    ...
    measured_mag_limits: list[np.ndarray] = []
    measured_phase_limits: list[np.ndarray] = []
    ...
    for index, session in enumerate(ready):
        ...
        plot_omega, plot_mag_db, plot_phase_deg = presentation_bode_series(result)
        ny_real, ny_imag = _nyquist_for_plot(result, phase_display)

        _append_finite(measured_mag_limits, plot_mag_db)
        _append_finite(measured_phase_limits, plot_phase_deg)
        _plot_line(ax_mag, plot_omega, plot_mag_db, color=color, lw=linewidth, alpha=alpha, label=label, semilogx=True)
        _plot_line(ax_phase, plot_omega, plot_phase_deg, color=color, lw=linewidth, alpha=alpha, label=label, semilogx=True)

        if is_primary:
            if len(ny_real):
                ax_ny.plot(ny_real[0], ny_imag[0], "o", color="#16a34a", ms=6, label="低频")
                ax_ny.plot(ny_real[-1], ny_imag[-1], "x", color="#dc2626", ms=7, label="高频")
                cutoff_idx = int(np.clip(getattr(result, "cutoff_index", 0), 0, len(ny_real) - 1))
                ax_ny.plot(ny_real[cutoff_idx], ny_imag[cutoff_idx], "*", color="#111827", ms=11, label="特征点")
        ax_ny.plot(ny_real, ny_imag, color=color, lw=linewidth, alpha=alpha, label=label)
    ...
    for session in ready:
        ...
        phase_display = phase_array_for_display(result.phase_deg, result.filter_type)
        ny_real, ny_imag = _nyquist_for_plot(result, phase_display)
        ny_mask = np.isfinite(ny_real) & np.isfinite(ny_imag)
        if len(ny_real) and np.any(ny_mask):
            ny_x.append(np.asarray(ny_real[ny_mask], dtype=float))
            ny_y.append(np.asarray(ny_imag[ny_mask], dtype=float))
    ...
    _set_y_limits_from_measured(ax_mag, measured_mag_limits, min_span=8.0, margin_fraction=0.08)
    _set_y_limits_from_measured(ax_phase, measured_phase_limits, min_span=30.0, margin_fraction=0.10)
