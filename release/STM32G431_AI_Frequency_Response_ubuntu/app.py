...
class MatlabExactApp:
    ...
    def _build_plot_area(self, parent):
        ...
        self.fig = Figure(figsize=(11.8, 7.2), dpi=100, facecolor="#f8fafc", constrained_layout=True)
    ...
    def _build_diagnosis_sidebar(self, parent):
        ...
        for row, key in enumerate(summary_keys):
            ...
            ttk.Label(diagnosis_box, textvariable=var, style=value_style, wraplength=360).grid(
                row=row, column=1, sticky="ew", padx=(10, 0), pady=3
            )
        diagnosis_actions = ttk.Frame(diagnosis_box, style="Surface.TFrame")
        diagnosis_actions.grid(row=len(summary_keys), column=0, columnspan=2, sticky="ew", pady=(8, 0))
    ...
    def _open_plot_zoom(self, kind: str):
        ...
        fig = Figure(figsize=(10.5, 6.8), dpi=100, facecolor="#f8fafc", constrained_layout=False)
        if kind == "bode":
